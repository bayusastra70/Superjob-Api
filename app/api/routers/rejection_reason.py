...
@router.get("/", response_model=List[RejectionReasonResponse])
...
@router.post("/", response_model=RejectionReasonResponse, status_code=status.HTTP_201_CREATED)
async def create_rejection_reason(
    reason: RejectionReasonCreate, 
    db: AsyncSession = Depends(get_db)
):
    """
