...
class UserService:
        ...
        try:
            ...
            if hasattr(user, 'keys'):
                ...
                user_data = {
                    ...
                    "updated_at": user[8],
                    "company_id": user[9],
                    "role": user[10] if len(user) > 10 else "candidate",
                    "default_role_id": user[11] if len(user) > 11 else 3
                }
            
            return user_data
            
        except Exception as e:
            logger.error(f"Error getting user profile with RBAC for user {user_id}: {str(e)}")
            return None
    ...
    def has_permission(self, user_id: int, permission_code: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking permission {permission_code} for user {user_id}: {str(e)}")
            return False
        finally:
            if cursor:
                cursor.close()
